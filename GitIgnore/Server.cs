...
{
    public JsonNode Resolve(IReadOnlyDictionary<string, object?> variables, IEnumerable<IPreParsedGraphQLSelection> selections)
    {
...
else if (arg.Value is PreParsedGraphQLVariableValue graphqlVariable)
{
    ...
    {
        throw new ArgumentException($"{graphqlVariable.Name} is not specified");
    }
...
else
{
    ...
    jsonNode[fieldSelection.Alias ?? fieldSelection.Name] = GetHasIgnoreFile(folder);
}
            }
...
{
    pattern = 
literal.Value.Select(x => {
IReadOnlyList<string>? patternElement = null;
if (x is PreParsedGraphQLStringValue literal)
{
    patternElement = ;
}
else if (x is PreParsedGraphQLNullValue)
{
    patternElement = null;
}
else if (x is PreParsedGraphQLVariableValue graphqlVariable)
{
    ...
    patternElement = ((JsonElement?)result)?;
}
else
{
    throw new ArgumentException("patternElement is not specified");
}
...
else if (arg.Value is PreParsedGraphQLVariableValue graphqlVariable)
{
    ...
    pattern = ((JsonElement)result).EnumerateArray().Select(x => x!).ToList();
}
}

}
    jsonNode[fieldSelection.Alias ?? fieldSelection.Name] = AddIgnorePattern(folder, pattern ?? throw new ArgumentNullException("pattern is null"));
}
            }
            else { throw new NotImplementedException(); }
        }
        ...
        if (parsedRequest.Query.Operation.OperationType == PreParsedGraphQLOperationType.Query)
{
    ...
    jsonNode = obj.Resolve(parsedRequest.Variables ?? new(), parsedRequest.Query.Operation.Selections);
}if (parsedRequest.Query.Operation.OperationType == PreParsedGraphQLOperationType.Mutation)
{
