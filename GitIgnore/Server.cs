                        ...
                        if (arg.Name == "pattern")
                        {
                            if (arg.Value is PreParsedGraphQLListValue literal)
                            {
                                pattern = literal
                                    .Value.Select(x =>
                                    {
                                        ...
                                        if (x is PreParsedGraphQLListValue literal)
                                        {
                                            patternElement = literal
                                                .Value.Select(x =>
                                                {
                                                    string? patternElementElement = null;
                                                    if (x is PreParsedGraphQLStringValue literal)
                                                    {
                                                        patternElementElement = literal.Value;
                                                    }
                                                    else if (
                                                        x
                                                        is PreParsedGraphQLVariableValue graphqlVariable
                                                    )
                                                    {
                                                        if (
                                                        ...
                                                        if (result is null)
                                                        {
                                                            throw new ArgumentNullException(
                                                                "patternElementElement is null"
                                                            );
                                                        }
                                                        patternElementElement = (
                                                            (JsonElement)result
                                                        ).GetString();
                                                    }
                                                    if (patternElementElement is null)
                                                    {
                                                        throw new ArgumentException(
                                                            "patternElementElement is not specified"
                                                        );
                                                    }
                                                    return patternElementElement;
                                                })
                                                .ToList();
                                        }
                            ...
                            else if (arg.Value is PreParsedGraphQLVariableValue graphqlVariable)
                            {
                                ...
                                pattern = ((JsonElement)result)
                                    .EnumerateArray()
                                    .Select(x =>
                                        x.EnumerateArray().Select(x => x.GetString()!).ToList()!
                                    )
                                    .ToList();
                            }
                        }
