...
{
    public JsonNode Resolve(IReadOnlyDictionary<string, object?> variables, IEnumerable<IGraphQLSelection> selections)
    {
...
else if (arg.Value is GraphQLVariableValue graphqlVariable)
{
    ...
    jsonNode[fieldSelection.Alias ?? fieldSelection.Name] = GetAddIgnorePattern(folder, pattern);
}
            }
...
public class GraphQLRequest
{
    ...
    {
        return JsonSerializer.Deserialize(requestBody, GraphQLServerJsonSerializerContext.Default.GraphQLRequest);
    }

    public string ToJsonString()
    {
        return JsonSerializer.Serialize(this, GraphQLServerJsonSerializerContext.Default.GraphQLRequest);
    }
}
        ...
        if (parsedRequest.Query.Operation.OperationType == GraphQLOperationType.Mutation)
{
    ...
    jsonNode = obj.Resolve(parsedRequest.Variables, parsedRequest.Query.Operation.Selections);
}
